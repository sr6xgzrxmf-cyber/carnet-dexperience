...
function normalizeCoverSrc(v: unknown): string | null {
  ...
  return (
    <div className="relative h-56 w-full overflow-hidden">
      <div className={`grid h-full w-full ${gridClass} gap-[2px] bg-black/30`}>
        {c.map((src, idx) => {
          const spanForFive = count === 5 && idx === 4 ? "row-span-2 col-span-1" : "";
          return (
            <div
              key={`${src}-${idx}`}
...
export default async function ActionsHubPage(props: {
  ...
  const seriesCards = featuredSeriesList.map((s) => {
    ...
    const covers = items.map((a) => normalizeCoverSrc(a.cover)).filter(Boolean) as string[];
  ...
  function hrefFor(nextTags: string[], nextShowAll = showAllTags) {
    ...
    return `/actions/hub${params.toString() ? `?${params.toString()}` : ""}`;
  }
    ...
    <div className="space-y-14">
        ...
        <div className="flex items-end justify-between gap-4">
          <div>
            ...
            <p className="mt-2 text-sm text-neutral-600 dark:text-neutral-400">
              Quelques entrées éditoriales. Des rétrospectives qui se lisent comme des séries.
            </p>
          </div>

          <Link
            href="/actions/par-ou-commencer"
            className="rounded-xl border border-neutral-200 dark:border-neutral-800 bg-white px-4 py-2 text-sm text-neutral-900 hover:bg-neutral-50 dark:bg-neutral-950/30 dark:text-neutral-200 dark:hover:bg-neutral-950/50"
          >
            Voir la page
          </Link>
        </div>
        ...
        <div className="mt-6 grid grid-cols-1 gap-6 md:grid-cols-2">
          {seriesCards.map((s) => {
            ...
            return (
              ...
              >
                {s.covers.length ? <Mosaic covers={s.covers} /> : <div className="h-56 w-full bg-neutral-900/10 dark:bg-neutral-900/30" />}
                ...
                <div className="p-6">
                  ...
                  <div className="mt-5 flex flex-wrap items-center gap-3">
                    <Link
                      href={startHref}
                      className="inline-flex items-center justify-center rounded-xl border border-neutral-200 dark:border-neutral-800 bg-white px-4 py-2 text-sm text-neutral-900 hover:bg-neutral-50 dark:bg-neutral-950/40 dark:text-neutral-200 dark:hover:bg-neutral-950/60"
                    >
      ...
      <section className="rounded-2xl border border-neutral-200 dark:border-neutral-800 bg-white/70 dark:bg-neutral-950/15 p-6 sm:p-8">
        ...
        <div className="mt-5 flex flex-wrap gap-2">
          {(showAllTags ? sortedTags : sortedTags.slice(0, 12)).map(({ tag, count }) => {
            ...
            return (
              <Link
                ...
                className={[
                  ...
                  active
                    ? "border-neutral-900/10 bg-neutral-900/5 text-neutral-900 dark:border-white/15 dark:bg-white/10 dark:text-neutral-100"
                    : "border-neutral-200 bg-white text-neutral-700 hover:bg-neutral-50 dark:border-neutral-800 dark:bg-neutral-950/30 dark:text-neutral-300 dark:hover:bg-neutral-950/50",
                ].join(" ")}
              >
